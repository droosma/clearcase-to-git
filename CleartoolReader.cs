...
namespace GitImporter
{
    public class CleartoolReader : IDisposable
    {
        ...
        public VobDb VobDb => new VobDb(ElementsByOid, _labelMetas);
        ...
        private void ReadElement(string elementName, bool isDir, Cleartool cleartool)
        {
            ...
            Logger.TraceData(TraceEventType.Start | TraceEventType.Verbose,
                             (int)TraceId.ReadCleartool,
                             "Start reading " + (isDir ? "directory" : "file") + " element",
                             elementName);
        ...
        private bool AddVersionToBranch(ElementBranch branch,
                                        int versionNumber,
                                        bool isDir,
                                        List<ElementVersion> newVersions,
                                        Cleartool cleartool)
        {
            ...
            if(version.Date > _originDate)
            {
                Logger.TraceData(TraceEventType.Information,
                                 (int)TraceId.ReadCleartool,
                                 $"Skipping version {version} : {version.Date} > {_originDate}");
                return false;
            }
        ...
        private void ReadVersion(string version, List<ElementVersion> newVersions, Cleartool cleartool)
        {
            ...
            lock(ElementsByOid)
                ...
                else if(element.Name != elementName)
                {
                    ...
                    Logger.TraceData(TraceEventType.Information,
                                     (int)TraceId.ReadCleartool,
                                     $"element with oid {oid} has a different name : now using {elementName} instead of {element.Name}");
