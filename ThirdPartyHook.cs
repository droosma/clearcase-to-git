...
namespace GitImporter
{
    public class ThirdPartyHook
    {
        ...
        public ThirdPartyHook(ThirdPartyConfig config)
        {
            ...
            PreWritingHooks = new List<GitWriter.PreWritingHook> {new GitWriter.PreWritingHook(new Regex(config.ProjectFileRegex), ProcessProjectFile)};
            PostWritingHooks = new List<GitWriter.PostWritingHook> {new GitWriter.PostWritingHook(new Regex(config.ConfigSpecRegex), ProcessConfigSpec)};
        }
        ...
        public string ModulesFile => _gitModulesFile ?? (_gitModulesFile = CreateGitModulesFile());
        ...
        public void ProcessConfigSpec(string configSpec, StreamWriter writer)
        {
            ...
            using(var stream = new StreamReader(configSpec))
                while((line = stream.ReadLine()) != null)
                {
                    ...
                    {
                        GitWriter.Logger.TraceData(TraceEventType.Warning,
                                                   (int)TraceId.ApplyChangeSet,
